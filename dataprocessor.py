...
class TerroristData:
    ...
    def get_lat_long(self, year=None):
        if year is not None:
            ...
            df_all = pd.read_sql_query("SELECT eventid, country_txt, iyear, longitude,latitude, success, nkill,  attacktype1_txt, weaptype1_txt from attacks"
                                       " where iyear <= {}".format(year), self.conn)
        else:
            df_all = pd.read_sql_query("SELECT eventid, country_txt, iyear, longitude,latitude,success, nkill,  attacktype1_txt, weaptype1_txt from attacks", self.conn)
        return df_all
    ...
    def get_weapon_data(self, eventids=[]):
        if len(eventids)==0:
            df_all = pd.read_sql_query("SELECT eventid, weaptype1_txt, iyear, success, country_txt from attacks", self.conn)
        else:
            df_all = pd.read_sql_query(f"SELECT eventid, weaptype1_txt, iyear, success, country_txt from attacks WHERE eventid IN ({','.join(eventids)})", self.conn)
        return df_all
