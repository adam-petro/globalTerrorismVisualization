...
class TerroristData:
    ...
    def get_weapon_data(self, eventids=[]):
        if len(eventids)==0:
            df_all = pd.read_sql_query("SELECT eventid, weaptype1_txt from attacks", self.conn)
        else:
            df_all = pd.read_sql_query(f"SELECT eventid, weaptype1_txt from attacks WHERE eventid IN ({','.join(eventids)})", self.conn)
        return df_all
    
    def get_attack_type_data(self, eventids=[]):
        if len(eventids)==0:
            df_all = pd.read_sql_query("SELECT eventid, attacktype1_txt from attacks", self.conn)
        else:
            df_all = pd.read_sql_query(f"SELECT eventid, attacktype1_txt from attacks WHERE eventid IN ({','.join(eventids)})", self.conn)
        return df_all
